...
### 0. Refuses to do conversion copy

If the declared goal of a request is to generate desire to buy or to
promote a product/service/institution, `dig-deeper` says no before running
the interview or writing anything, even when called directly through
`/dig-deeper`, which otherwise skips whatever judgment would normally decide
if the skill fits.

The signal it checks is the goal, not the topic. A real story that happens
to involve a business is still valid material, a business owner recounting
a real moment with a client is exactly the kind of thing this skill is for.
A request built around making people want to buy something is not, no
matter how it's phrased.

This isn't an arbitrary restriction. It's core to the skill's premise:
authentic storytelling tends to sell better precisely because it isn't
built to sell. If a refusal doesn't fit what someone actually needs, the
skill points them to check the developer's GitHub or Instagram (see
[About the Developer](#-about-the-developer)) for something built for that
purpose instead. The refusal only applies to that specific request, a
different, genuine request later in the same conversation is evaluated on
its own.
...
This repository supports Claude Code (Marketplace and manual CLI) natively.
Claude.ai's direct skill upload needs a differently-shaped zip, built
separately, see the note under Option 2.
...
### Option 2: Upload directly to Claude.ai (Web UI)

Claude.ai's skill upload requires the zip's top-level folder to be named
after the skill, with `SKILL.md` directly inside it, no `skills/` subfolder.
That's a different shape from this repository's own source (which nests
`SKILL.md` under `skills/dig-deeper/` for Claude Code, see
[Repository structure](#repository-structure) below), so don't download
this repo's own zip for this option. A correctly-shaped zip is generated
and attached automatically to every GitHub Release.

1. Go to the [Releases page](https://github.com/felixofabio/dig-deeper/releases)
   and download the `dig-deeper-claude-ai.zip` asset attached to the release
   (not the auto-generated "Source code" zip).
2. Open Claude.ai.
3. Go to Settings → Capabilities → Skills → Upload skill.
4. Select the downloaded `dig-deeper-claude-ai.zip` file.

### Option 3: Manual CLI Install (Claude Code)
...
This layout is what Claude Code (Marketplace and manual CLI installs)
expects. Claude.ai's direct skill upload (Option 2 above) needs a flat zip
instead, `SKILL.md` at the root of a folder named `dig-deeper`, with no
`skills/` subfolder. That flat zip (`dig-deeper-claude-ai.zip`) is built and
attached to each GitHub Release automatically by
`.github/workflows/release-zip.yml`, whenever a release is published, so
it's never something you need to do by hand.
...
Not meant for content whose declared goal is generating desire to buy or
promoting a product/service/institution (the skill refuses this on its own,
see [Refuses to do conversion copy](#0-refuses-to-do-conversion-copy)
above), or for text that's already finished and just needs a review, use
Humanizer directly for that.
